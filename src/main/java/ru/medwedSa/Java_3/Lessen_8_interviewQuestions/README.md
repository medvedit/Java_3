...
* > * `Iterator` а также `ListIterator` два важных интерфейса Java Collection Framework, где ListIterator расширяет 
      ...
      1. **Основное различие между Iterator а также ListIterator:**  
      С `Iterator`, мы можем двигаться только вперед, используя его hasNext() а также next() метод в то время как с 
