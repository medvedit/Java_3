...
+ [Какими значениями по умолчанию инициализируются поля примитивных типов данных?](#4-какими-значениями-по-умолчанию-инициализируются-поля-примитивных-типов-данных-)
+ [Почему Java не поддерживает указатели, множественное наследование?](#5-почему-java-не-поддерживает-указатели-множественное-наследование-)
+ [Что такое циклы? Какие типы циклов вы знаете?](#6-что-такое-циклы-какие-типы-циклов-вы-знаете)
+ [Как можно организовать бесконечный цикл?](#7-как-можно-организовать-бесконечный-цикл)
+ [Что такое break, continue и label?](#8-что-такое-break-continue-и-label)
+ [Что такое пакеты, зачем они нужны? Какие пакеты по умолчанию подключены?](#9-что-такое-пакеты-зачем-они-нужны-какие-пакеты-по-умолчанию-подключены)
+ [Импортируются ли подпакеты при импорте пакета?](#10-импортируются-ли-подпакеты-при-импорте-пакета)
## ООП:
+ [Что такое классы / объекты?](#1-что-такое-классы--объекты)
+ [Что такое инкапсуляция, наследование и полиморфизм?](#2-что-такое-инкапсуляция-наследование-и-полиморфизм)
+ [Что такое абстрактный класс?](#3-что-такое-абстрактный-класс)
+ [Можно ли создавать абстрактные методы в обычном классе? Создавать обычное методы в абстрактном классе?](#4-можно-ли-создавать-абстрактные-методы-в-обычном-классе-создавать-обычное-методы-в-абстрактном-классе)
+ [Что такое интерфейс?](#5-что-такое-интерфейс)
+ [Бывают ли пустые интерфейсы?](#6-бывают-ли-пустые-интерфейсы)
+ [Можно ли создать объект: Animal animal = new Flyable(); Если Animal является интерфейсом.](#7-можно-ли-создать-объект-animal-animal--new-flyable-если-animal-является-интерфейсом)
+ [Что означает ключевое слово static? Можно ли переопределить private или static - методы?](#8-что-означает-ключевое-слово-static-можно-ли-переопределить-private-или-static---методы)
+ [Можно ли получить доступ к нестатическому полю из статического метода?](#9-можно-ли-получить-доступ-к-нестатическому-полю-из-статического-метода)
+ [Что такое блок static {}?](#10-что-такое-блок-static-)
+ [Что такое cast?](#11-что-такое-cast)
+ [Что такое переопределение и перегрузка методов?](#12-что-такое-переопределение-и-перегрузка-методов-)
+ [Что такое конструктор и перегрузка конструктора? Может ли класс иметь несколько конструкторов?](#13-что-такое-конструктор-и-перегрузка-конструктора-может-ли-класс-иметь-несколько-конструкторов)
+ [Когда вызывается конструктор?](#14-когда-вызывается-конструктор)
+ [Поддерживает ли Java множественное наследование?](#15-поддерживает-ли-java-множественное-наследование)
+ [Какие модификаторы доступа вы знаете и сколько их?](#16-какие-модификаторы-доступа-вы-знаете-и-сколько-их)
+ [Что означает ключевое слово this?](#17-что-означает-ключевое-слово-this)
+ [Что означает ключевое слово super?](#18-что-означает-ключевое-слово-super)
+ [Что означает ключевое слово final?](#19-что-означает-ключевое-слово-final)
+ [Какой класс является базовым для всех классов?](#20-какой-класс-является-базовым-для-всех-классов)
...
## Исключения:
+ [Что означает каждое из ключевых слов: try, catch, finally, throw, throws?](#1-что-означает-каждое-из-ключевых-слов-try-catch-finally-throw-throws)
+ [Для чего в основном используется блок finally?](#2-для-чего-в-основном-используется-блок-finally)
+ [Что такое try-with-resource?](#3-что-такое-try-with-resource)
+ [Покажите примерную схему дерева классов, наследующихся от Throwable;](#4-покажите-примерную-схему-дерева-классов-наследующихся-от-throwable)
+ [Что такое checked и unchecked исключения?](#5-что-такое-checked-и-unchecked-исключения)
+ [Что происходит при необработанном исключении?](#6-что-происходит-при-необработанном-исключении)
+ [В чем разница между Exception и Error?](#7-в-чем-разница-между-exception-и-error)
+ [Может ли исключение быть снова брошено после перехвата?](#8-может-ли-исключение-быть-снова-брошено-после-перехвата)
## Многопоточность:
+ [В чем разница между процессом и потоком?](#1-в-чем-разница-между-процессом-и-потоком)
+ [Какие существуют способы создания потоков (и сколько их)?](#2-какие-существуют-способы-создания-потоков-и-сколько-их)
+ [Какой способ создания потоков предпочтителен?](#3-какой-способ-создания-потоков-предпочтителен)
+ [Что такое Deadlock?](#4-что-такое-deadlock)
+ [В чем разница между notify() и notifyAll()?](#5-в-чем-разница-между-notify-и-notifyall)
+ [В чем разница между sleep() и wait()?](#6-в-чем-разница-между-sleep-и-wait)
+ [Можно ли перезапустить завершившийся поток?](#7-можно-ли-перезапустить-завершившийся-поток)
+ [Какие способы синхронизации вы знаете?](#8-какие-способы-синхронизации-вы-знаете)
+ [Зачем нужен Semaphore, ExecutorService?](#9-зачем-нужен-semaphore-executorservice)
+ [Что такое daemon thread?](#10-что-такое-daemon-thread)
## JDBC:
+ [Что такое JDBC?](#1-что-такое-jdbc)
+ [Какова роль драйвера в JDBC?](#2-какова-роль-драйвера-в-jdbc)
+ [Зачем нужен вызов метода Class.forName(...)?](#3-зачем-нужен-вызов-метода-classforname)
+ [В чем разница между Statement и PreparedStatement?](#4-в-чем-разница-между-statement-и-preparedstatement)
+ [Что такое Connection Pool?](#5-что-такое-connection-pool)
+ [Что такое ResultSet?](#6-что-такое-resultset)
## Дополнительные вопросы:
+ [Что такое обобщения?](#1-что-такое-обобщения)
+ [В чем разница между == и equals()?](#2-в-чем-разница-между--и-equals)
+ [Что такое сериализация/десериализация?](#3-что-такое-сериализациядесериализация)
+ [Что за ключевые слова transient и volatile?](#4-что-за-ключевые-слова-transient-и-volatile)
+ [В чем разница между Serializable и Externalizable?](#5-в-чем-разница-между-serializable-и-externalizable)
+ [Необходим ли установленный JDK для запуска Java-программ?](#6-необходим-ли-установленный-jdk-для-запуска-java-программ)
+ [Можно ли в интерфейсе объявить статический метод?](#7-можно-ли-в-интерфейсе-объявить-статический-метод)
+ [Можно ли увеличить размер массива после его инициализации?](#8-можно-ли-увеличить-размер-массива-после-его-инициализации)
+ [Что такое локальный класс?](#9-что-такое-локальный-класс)
+ [Что такое сборщик мусора (Garbage Collector)?](#10-что-такое-сборщик-мусора-garbage-collector)
+ [Что означает иммутабельность String?](#11-что-означает-иммутабельность-string)
+ [Как создать собственный иммутабельный класс?](#12-как-создать-собственный-иммутабельный-класс)
+ [В чем разница между набором классов Reader/Writer и InputStream/OutputStream?](#13-в-чем-разница-между-набором-классов-readerwriter-и-inputstreamoutputstream)
# Основы:
...
обязательно помечайте его аннотацией _@Override_!  
Читаем и смотрим [тут...](https://ru.hexlet.io/courses/java_101/lessons/override_in_java/theory_unit)
...
*  > * `Может иметь несколько конструкторов`. Нет ограничений на количество конструкторов.
   >
   [Содержание](#ооп)
...
* > * `this` - может использоваться внутри любого метода для ссылки на текущий объект. То есть this всегда служит 
...
* > * `super` - это ссылка на объект родительского типа. _**super**_ обозначает суперкласс, т. е. класс, производным от
...
* > * `final` - Модификатор final - предназначен для создания констант. Если мы пропишем **_final_** перед типом данных 
...
[Содержание](#ооп)
# Коллекции:
`В моем, личном репозитории, есть форкнутый репозиторий java-interview. По адресу java-interview /jcf.md доп ответы по коллекциям.`
...
* > * `Обобщения (Generic)` - это параметризованные типы. С их помощью можно объявлять классы, интерфейсы и методы, 
   ...
   >
[Содержание](#дополнительные-вопросы)
