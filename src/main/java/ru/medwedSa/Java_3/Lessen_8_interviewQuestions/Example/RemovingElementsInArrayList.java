package ru.medwedSa.Java_3.Lessen_8_interviewQuestions.Example;

import java.util.ArrayList;
import java.io.*;

public class RemovingElementsInArrayList {
        ...
        public static void main(String[] args){
            ...
            System.out.println("Размер копии списка:" + copyList.size());
            System.out.println("СБОРКА УСПЕШНО ЗАВЕРШЕНА (общее время: " + finalTime + " миллисекунды)");
        }

        private static void removeEfficiently() {
            /* если необходимо удалить все элементы, начиная с указанного,
             * то удаляем элементы с конца списка до m
             */
            if (m + n >= size){
                int i = size - 1;
                while (i != m - 1){
